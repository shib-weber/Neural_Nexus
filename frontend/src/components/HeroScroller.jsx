...
export default function HeroScroller() {
  ...
  // 1. Preload individual high-res image frames into memory
  useEffect(() => {
    const loadedImages = []
    let isMounted = true
  ...
  // 3. Render crisp high-DPI frames onto the canvas
  useEffect(() => {
  ...
  const isReady = loadedCount >= TOTAL_FRAMES * 0.65 // Render smoothly once 65% is ready

  return (
    <div ref={containerRef} className="relative h-[350vh] bg-[#050a07]" id="home">
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center bg-[#050a07]">
        ...
        {/* Professional Loading Screen (Prevents Black Flashes) */}
        {!isReady && (
        ...
        <div className="absolute inset-0 z-20 w-full max-w-[1240px] mx-auto px-4 sm:px-[clamp(16px,3.4vw,40px)] flex flex-col justify-end pb-10 sm:pb-16 pointer-events-none">
          
          {/* Initial Heading (Bottom-Left Aligned) */}
{/* Content Container */}
        <div className="absolute inset-0 z-20 w-full max-w-[1240px] mx-auto px-4 sm:px-[clamp(16px,3.4vw,40px)] flex flex-col justify-between pt-28 sm:pt-32 pb-12 pointer-events-none">
          
          {/* Initial Heading anchored at the TOP-LEFT */}
          <div 
          ...
          <div 
            className="absolute inset-x-4 inset-y-0 max-w-4xl mx-auto flex items-center justify-center transition-all duration-500 pointer-events-auto"
            style={{
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full px-2">
              {/* Card 1 */}
              <div className="p-5 sm:p-6 rounded-2xl bg-[rgba(12, 20, 16, 0.49)] border border-cyan-500/40 backdrop-blur-2xl shadow-2xl shadow-cyan-950/80 text-left">
              ...
              {/* Card 2 */}
              <div className="p-5 sm:p-6 rounded-2xl bg-[rgba(12, 20, 16, 0.49)o-500/40 backdrop-blur-2xl shadow-2xl shadow-indigo-950/80 text-left">
